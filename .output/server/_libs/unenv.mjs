...
import { a as init_utils, i as createNotImplementedError } from "./detect-libc+unenv.mjs";
import { c as noop_default, s as init_noop } from "./miniflare.mjs";
import { n as resolveAlias, t as init_utils$1 } from "./pathe.mjs";
...
//#region node_modules/unenv/dist/runtime/node/internal/v8/deserializer.mjs
var Deserializer, DefaultDeserializer;
var init_deserializer = __esmMin((() => {
	Deserializer = class {
		readHeader() {
			return false;
		}
	...
	DefaultDeserializer = class extends Deserializer {};
}));
//#endregion
//#region node_modules/unenv/dist/runtime/node/internal/v8/serializer.mjs
...
var init_serializer = __esmMin((() => {
	...
	DefaultSerializer = class extends Serializer {};
}));
//#endregion
//#region node_modules/unenv/dist/runtime/node/internal/v8/profiler.mjs
var GCProfiler;
...
//#endregion
//#region node_modules/unenv/dist/runtime/node/v8.mjs
function queryObjects(_ctor, options) {
	if (options?.format === "count") return 0;
	return [];
}
var getMockHeapSpaceStats, cachedDataVersionTag, deserialize, getHeapCodeStatistics, getHeapSpaceStatistics, getHeapStatistics, getHeapSnapshot, promiseHooks, serialize, setFlagsFromString, setHeapSnapshotNearHeapLimit, startupSnapshot, stopCoverage, takeCoverage, writeHeapSnapshot, v8_default;
var init_v8 = __esmMin((() => {
	init_noop();
	init_deserializer();
	init_serializer();
	init_profiler();
	getMockHeapSpaceStats = (name) => ({
		space_name: name,
		space_size: 0,
		space_used_size: 0,
	...
	setFlagsFromString = noop_default;
	setHeapSnapshotNearHeapLimit = noop_default;
	startupSnapshot = {
	...
	stopCoverage = noop_default;
	takeCoverage = noop_default;
	writeHeapSnapshot = () => "";
	v8_default = {
		DefaultDeserializer,
		Deserializer,
...
//#endregion
//#region node_modules/unenv/dist/index.mjs
var dist_exports = /* @__PURE__ */ __exportAll({ defineEnv: () => defineEnv });
...
function emitInvalidSegmentDeprecation(target, request, match, packageJsonUrl, internal, base, isTarget) {
	if (processModule.noDeprecation) return;
	const pjsonPath = fileURLToPath(packageJsonUrl);
	const double = doubleSlashRegEx.exec(isTarget ? target : request) !== null;
	processModule.emitWarning(`Use of deprecated ${double ? "double slash" : "leading or trailing slash matching"} resolving "${target}" for module request "${request}" ${request === match ? "" : `matched to "${match}" `}in the "${internal ? "imports" : "exports"}" field module resolution of the package at ${pjsonPath}${base ? ` imported from ${fileURLToPath(base)}` : ""}.`, "DeprecationWarning", "DEP0166");
}
function emitLegacyIndexDeprecation(url, packageJsonUrl, base, main) {
	...
	if (!main) processModule.emitWarning(`No "main" or "exports" field defined in the package.json for ${packagePath} resolving the main entry point "${urlPath.slice(packagePath.length)}", imported from ${basePath}.
Default "index" lookups for the main are deprecated for ES modules.`, "DeprecationWarning", "DEP0151");
	else if (path.resolve(packagePath, main) !== urlPath) processModule.emitWarning(`Package ${packagePath} has a "main" field set to "${main}", excluding the full filename and extension to the resolved file at "${urlPath.slice(packagePath.length)}", imported from ${basePath}.
...
function emitTrailingSlashPatternDeprecation(match, pjsonUrl, base) {
	...
	processModule.emitWarning(`Use of deprecated trailing slash pattern mapping "${match}" in the "exports" field module resolution of the package at ${pjsonPath}${base ? ` imported from ${fileURLToPath(base)}` : ""}. Mapping specifiers ending in "/" is no longer supported.`, "DeprecationWarning", "DEP0155");
}
...
function unenvPreset(opts) {
	...
	if (opts.nodeCompat !== false) {
		...
		Object.assign(preset.alias, { ...Object.fromEntries(Object.entries(nodeCompatAliases).flatMap(([from, to]) => {
			return from.startsWith("node:") ? [[from, to]] : [[from, to], [`node:${from}`, to]];
		})) });
	}
...
function resolvePaths(env, from, opts = {}) {
	...
	const { resolveModulePath: resolveModulePath$1 } = createResolver({ from: [
	...
	const _resolve = (id) => {
		...
		let resolved = resolveModulePath$1(id, { try: true });
		if (!resolved && id.startsWith("unenv/")) resolved = resolveModulePath$1(id.replace("unenv/", "unenv-nightly/"), { try: true });
		return resolved || id;
	};
	...
	for (const global in env.inject) {
		...
		if (Array.isArray(inject)) {
			const [id, ...path$1] = inject;
			env.inject[global] = [_resolve(id), ...path$1];
...
var init_dist = __esmMin((() => {
	...
	isWindows = /* @__PURE__ */ (() => processModule.platform === "win32")();
...
//#endregion
//#region node_modules/unenv/dist/runtime/node/internal/domain/domain.mjs
var Domain;
var init_domain$1 = __esmMin((() => {
	init_utils();
	Domain = class extends EventEmitter {
		...
		remove() {}
		bind(callback) {
			throw /* @__PURE__ */ createNotImplementedError("Domain.bind");
		}
		intercept(callback) {
			throw /* @__PURE__ */ createNotImplementedError("Domain.intercept");
		}
		run(fn, ...args) {
			throw /* @__PURE__ */ createNotImplementedError("Domain.run");
		}
	};
}));
//#endregion
//#region node_modules/unenv/dist/runtime/node/domain.mjs
...
var create, createDomain, _domain, active, _stack, domain_default;
var init_domain = __esmMin((() => {
	init_domain$1();
	create = function() {
		return new Domain();
	};
...
//#endregion
export { init_dist as i, init_domain as n, dist_exports as r, domain_exports as t };
