...
import { n as child_process_exports, r as init_child_process, t as require_detect_libc } from "./detect-libc+unenv.mjs";
import { n as require_gte, r as require_coerce, t as require_satisfies } from "./semver.mjs";
import { t as require_colour$1 } from "./img__colour.mjs";
...
var name, description, version, author, homepage, contributors, scripts, type, main, types, files, repository, keywords, dependencies, optionalDependencies, devDependencies, license, engines, config, funding, package_default;
...
var require_libvips = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	SPDX-License-Identifier: Apache-2.0
	*/
	var { spawnSync } = (init_child_process(), __toCommonJS(child_process_exports));
	var { createHash } = __require("node:crypto");
	var semverCoerce = require_coerce();
	var semverGreaterThanOrEqualTo = require_gte();
	var semverSatisfies = require_satisfies();
	var detectLibc = require_detect_libc();
	var { config, engines, optionalDependencies } = (init_package(), __toCommonJS(package_exports).default);
	var minimumLibvipsVersion = semverCoerce(processModule.env.npm_package_config_libvips || config.libvips).version;
	...
	var runtimePlatformArch = () => `${processModule.platform}${runtimeLibc()}-${processModule.arch}`;
	var buildPlatformArch = () => {
		/* node:coverage ignore next 3 */
		if (isEmscripten()) return "wasm32";
		const { npm_config_arch, npm_config_platform, npm_config_libc } = processModule.env;
		const libc = typeof npm_config_libc === "string" ? npm_config_libc : runtimeLibc();
		return `${npm_config_platform || processModule.platform}${libc}-${npm_config_arch || processModule.arch}`;
	};
	...
	var isUnsupportedNodeRuntime = () => {
		if (processModule.release?.name === "node" && processModule.versions) {
			if (!semverSatisfies(processModule.versions.node, engines.node)) return {
				found: processModule.versions.node,
				expected: engines.node
			};
		}
	};
	var isEmscripten = () => {
		const { CC } = processModule.env;
		return Boolean(CC?.endsWith("/emcc"));
	};
	var isRosetta = () => {
		if (processModule.platform === "darwin" && processModule.arch === "x64") return (spawnSync("sysctl sysctl.proc_translated", spawnSyncOptions).stdout || "").trim() === "sysctl.proc_translated: 1";
		return false;
	};
	...
	var globalLibvipsVersion = () => {
		if (processModule.platform !== "win32") return (spawnSync("pkg-config --modversion vips-cpp", {
			...spawnSyncOptions,
			env: {
				...processModule.env,
	...
	var pkgConfigPath = () => {
		if (processModule.platform !== "win32") return [
			...
			processModule.env.PKG_CONFIG_PATH,
	...
	var useGlobalLibvips = (logger) => {
		if (Boolean(processModule.env.SHARP_IGNORE_GLOBAL_LIBVIPS) === true) return skipSearch(false, "SHARP_IGNORE_GLOBAL_LIBVIPS", logger);
		if (Boolean(processModule.env.SHARP_FORCE_GLOBAL_LIBVIPS) === true) return skipSearch(true, "SHARP_FORCE_GLOBAL_LIBVIPS", logger);
		/* node:coverage ignore next 3 */
...
var require_utility = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	*/
	...
	versions.sharp = (init_package(), __toCommonJS(package_exports).default).version;
	/* node:coverage ignore next 5 */
