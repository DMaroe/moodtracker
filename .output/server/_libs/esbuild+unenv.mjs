import { i as __require, n as __esmMin, r as __exportAll, t as __commonJSMin } from "../_runtime.mjs";
import { a as init_utils, n as child_process_exports, o as notImplemented, r as init_child_process } from "./detect-libc+unenv.mjs";
...
//#endregion
//#region node_modules/wrangler/node_modules/esbuild/lib/main.js
	...
	function createChannel(streamIn) {
		...
		let handleIncomingPacket = (bytes) => {
			if (isFirstPacket) {
				...
				if (binaryVersion !== "0.28.1") throw new Error(`Cannot start service: Host version "0.28.1" does not match binary version ${quote(binaryVersion)}`);
				return;
			}
...
${file}:${line}:${column}: ERROR: ${e.pluginName ? `[plugin: ${e.pluginName}] ` : ""}${e.text}`;
	...
	var ESBUILD_BINARY_PATH = processModule.env.ESBUILD_BINARY_PATH || ESBUILD_BINARY_PATH;
	...
	function pkgAndSubpathForCurrentPlatform() {
		...
		let subpath;
		let isWASM = false;
		let platformKey = `${processModule.platform} ${os.arch()} ${os.endianness()}`;
...
for your current platform.`);
		...
		if (/\.zip\//.test(binPath)) {
			...
			if (pnpapi) {
				...
				const binTargetPath = path.join(root, "node_modules", ".cache", "esbuild", `pnpapi-${pkg.replace("/", "-")}-0.28.1-${path.basename(subpath)}`);
	...
	var worker_threads;
	if (processModule.env.ESBUILD_WORKER_THREADS !== "0") {
		try {
			worker_threads = (init_worker_threads(), __toCommonJS(worker_threads_exports));
		} catch {}
		let [major, minor] = processModule.versions.node.split(".");
...
More information: The file containing the code for esbuild's JavaScript API (${__filename}) does not appear to be inside the esbuild package on the file system, which usually means that the esbuild package was bundled into another file. This is problematic because the API needs to run a binary executable inside the esbuild package which is located using a relative path from the API code to the executable. If the esbuild package is bundled, the relative path will be incorrect and the executable won't be found.`);
	...
	var defaultWD = processModule.cwd();
	...
	var runServiceSync = (callback) => {
		...
		callback(service);
		readFromStdout(child_process.execFileSync(command, args.concat(`--service=0.28.1`), {
			cwd: defaultWD,
			windowsHide: true,
			input: stdin,
			maxBuffer: +processModule.env.ESBUILD_MAX_BUFFER || 16 * 1024 * 1024
		}));
		afterClose(null);
	};
	var randomFileName = () => {
...
//#endregion
export { tty_exports as a, init_tty as i, init_worker_threads as n, worker_threads_exports as r, require_main as t };
